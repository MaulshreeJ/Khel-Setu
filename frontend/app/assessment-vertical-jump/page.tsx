...
import { uploadVerticalJumpAssessment, getCurrentUser } from "@/lib/api"
import Link from "next/link"

export default function VerticalJumpAssessmentPage() {
  ...
  const handleUpload = async () => {
    ...
    setError(null)
    try {
      const result = await uploadVerticalJumpAssessment(videoBlob)
    ...
    <div className="min-h-screen bg-background p-4 md:p-8">
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">AI Vertical Jump Assessment</h1>
            <p className="text-muted-foreground">Record your jump for AI height estimation.</p>
          </div>
        </div>
              ...
              <Card>
                ...
                <CardContent className="space-y-4 text-sm text-muted-foreground">
                  <ol className="list-decimal list-inside space-y-2">
                    ...
                    <li>Stand still for 2-3 seconds so the AI can establish your baseline height.</li>
                    <li>Press "Start Recording".</li>
                    <li>Perform 2-3 maximum vertical jumps.</li>
                    <li>Press "Stop Recording" then "Analyze Video".</li>
                  </ol>
                  <div className="p-3 bg-yellow-50 text-yellow-800 rounded-md text-xs mt-4 border border-yellow-100">
                    <strong>Note:</strong> Height is estimated relative to your body proportions. Results are approximate.
                  </div>
                </CardContent>
              </Card>
            ) : (
              <Card className="border-green-200 bg-green-50/50">
                ...
                <CardContent className="space-y-6">
                  <div className="text-center p-6 bg-white rounded-xl shadow-sm border border-green-100">
                    <div className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-2">Estimated Jump Height</div>
                    <div className="text-6xl font-black text-green-600">{analysisResult.jump_height_cm ?? analysisResult.reps_counted}<span className="text-2xl ml-1">cm</span></div>
                  </div>
